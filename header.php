...
//Web Configuration settings Starts Here
$sqlwebconfig = "SELECT * FROM  webconfig";
$qsqlwebconfig = mysqli_query($con,$sqlwebconfig);
if(mysqli_num_rows($qsqlwebconfig) == 0)
{
	$sqlins = "INSERT INTO  webconfig(web_title,web_tagline,web_logo,favicon_logo) VALUES('Online Auction','Online Bidding Website','img/logo/logo_footer.png','img/favicon.ico')";
	mysqli_query($con,$sqlins);
	$sqlwebconfig = "SELECT * FROM  webconfig";
	$qsqlwebconfig = mysqli_query($con,$sqlwebconfig);
	$rswebconfig = mysqli_fetch_array($qsqlwebconfig);
}
else
{
	$rswebconfig = mysqli_fetch_array($qsqlwebconfig);
}
//Web Configuration settings Ends Here
//Web Page Config Starts here
$sqlwebpage = "SELECT * FROM  webpage";
$qsqlwebpage = mysqli_query($con,$sqlwebpage);
if(mysqli_num_rows($qsqlwebpage) == 0)
{
	$sqlinswebpage = "INSERT INTO  webpage(webpage_link,webpage_title,webpage_description,webpage_image) VALUES('about-us.php','About Online Auction','Online Auction - Online Bidding Website','images/online-auction.jpg')";
	mysqli_query($con,$sqlinswebpage);
	echo mysqli_error($con);
}
//Web Page Config Ends here
...
{
	$sql = "SELECT customer_id, customer_role, status FROM customer WHERE email_id=? AND password=? AND status='Active'";
	$stmt = mysqli_prepare($con, $sql);
	mysqli_stmt_bind_param($stmt, "ss", $_POST['emailid'], $_POST['password']);
	...
	if(mysqli_num_rows($result) == 1)
	{
		...
		$_SESSION['customer_role'] = $rs['customer_role'];
		
		// Check account balance
		$sql = "SELECT SUM(purchase_amount) FROM billing WHERE customer_id=? and status='Active' and payment_type='Deposit'";
		$stmt = mysqli_prepare($con, $sql);
		mysqli_stmt_bind_param($stmt, "i", $_SESSION['customer_id']);
		mysqli_stmt_execute($stmt);
		$balResult = mysqli_stmt_get_result($stmt);
		$rs = mysqli_fetch_array($balResult);
		$depamt = $rs[0];

		$sql = "SELECT SUM(paid_amount) FROM payment WHERE customer_id=? and status='Active' and payment_type='Bid'";
		$stmt = mysqli_prepare($con, $sql);
		mysqli_stmt_bind_param($stmt, "i", $_SESSION['customer_id']);
		mysqli_stmt_execute($stmt);
		$payResult = mysqli_stmt_get_result($stmt);
		$rs = mysqli_fetch_array($payResult);
		$widamt = $rs[0];
...
<head>
        ...
        <title><?php echo $rswebconfig['web_title']; ?> - <?php echo $rswebconfig['web_tagline']; ?></title>
        <meta name="description" content="<?php echo $rswebconfig['web_tagline']; ?>">
        <meta name="viewport" content="width=device-width, initial-scale=1">	
        <!-- Place favicon.ico in the root directory -->
	    <link rel="shortcut icon" type="image/x-icon" href="<?php echo $rswebconfig['favicon_logo']; ?>">	
		<!-- all CSS hear -->		
                    ...
                    <div class="container-fluid">
                        <div class="row">
                            <div class="col-lg-2 col-md-3 md-custom-12">
                                <!-- logo start -->
                                <div class="logo">
                                    <a href="index.php"><img src="img/logo/logo-2.png" alt="" style="width: 200px;height: 40px;"></a>
                                </div>
                                <!-- logo end -->
                            </div>
...
<li><a href="viewbillingcustomer.php">View Trasaction</a></li>
		...
		<?php
    // Only show My Services dropdown if customer role is NOT "Service Man"
    if(!isset($_SESSION['customer_role']) || $_SESSION['customer_role'] !== "Customer") {
    ?>
        <li class="drodown-show"><a href="#"> My Services <i class="fa fa-angle-down"></i></a>
            <ul class="open-dropdown setting">
                <li><a href="selectcategory.php">Request Services</a></li>
                <li><a href="viewproduct.php">View Services</a></li>
            </ul>
        </li>
    <?php
    }
	...
	?>
		...
		{
		?>
		<li class="drodown-show"><a href="#"> Settings <i class="fa fa-angle-down"></i></a>
			<ul class="open-dropdown setting">
				<li><a href="webconfig.php">Web Config</a></li>
				<li><a href="webpage.php">About Page</a></li>
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                ...
                                <div class="hot-line-area">
                                    <div class="phone">
                                        Customer Support: <span>Phone Number </span>
                                    </div>
                                </div>
                                <!-- hot-line-area end -->
...
<div class="item-total"><?php echo $currencysymbol; ?><?php echo $accbalamt; ?></div>
</a>
</li>
<?php
}
				...
				<div class="row">
					<div class="col">
						<div class="slider-text-info style-2 text-center slider-text-animation">
							<h1 class="title1"><span class="text">Service Auction..</span></h1>
							<h3>Bangladesh's top rated Service auction platform..</h3s>
